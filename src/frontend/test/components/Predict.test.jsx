...
describe("Predict — rendering", () => {
  ...
  it("shows the sub-heading copy", () => {
    render(<Predict />);
    expect(screen.getByText(/adjust values\. predict\./i)).toBeInTheDocument();
  });
  ...
  it("renders the endpoint URL", async () => {
    ...
    expect(screen.getByText(/\/api\/v1\/predict\/champion/)).toBeInTheDocument();
  });
});
...
describe("Predict — Import modal", () => {
  ...
  it("shows an error when applying fewer values than expected", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/expected 41 features/i)).toBeInTheDocument()
    );
  });
...
describe("Predict — prediction flow", () => {
  ...
  it("displays an error message when the API returns a non-OK status", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/unprocessable entity/i)).toBeInTheDocument()
    );
  });
  ...
  it("shows 'Predicting…' in the button while loading", async () => {
    ...
    act(() => {
      resolvePredict({
        ok: true,
        status: 200,
        json: () => Promise.resolve({ prediction: 0.5, model_uri: "", run_id: "" }),
      });
    });
  });
