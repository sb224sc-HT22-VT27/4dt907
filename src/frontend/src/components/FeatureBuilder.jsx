  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div
        ...
        aria-valuenow={value}
      >
        <svg className="ios-knob__ring" viewBox={`0 0 ${SIZE} ${SIZE}`} aria-hidden="true">
          <defs>
            <linearGradient id={`iosgrad-${gid}`} x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="#38bdf8" stopOpacity="0.95" />
              <stop offset="45%" stopColor="#a855f7" stopOpacity="0.88" />
              <stop offset="100%" stopColor="#fb7185" stopOpacity="0.80" />
            </linearGradient>
          </defs>
      ...
      <div className="text-xs font-semibold text-slate-800">{shortLabel(name)}</div>
    ...
    <div className="ios-card rounded-[28px] p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {groups.map((g) => (
          ...
          >
            {g.key} <span className="opacity-60">({g.names.length})</span>
          </button>
        ))}
      </div>
